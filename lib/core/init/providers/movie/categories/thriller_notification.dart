...
class ThrillerNotifier extends StateNotifier<Pagination> {
  ThrillerNotifier([Pagination? state]) : super(state ?? Pagination.initial()) {
    getMovies();
  }
  ...
  Future<List<Movie>?> fetchMovies([int page = 1]) async {
    ...
    final String url =
        "$baseUrl$genre$apiKey&with_genres=53&page=$page&language=$langCode";
...
final thrillerNotifier = StateNotifierProvider<ThrillerNotifier, Pagination>(
    (ref) => ThrillerNotifier());
