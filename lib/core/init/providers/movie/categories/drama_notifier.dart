...
class DramaNotifier extends StateNotifier<Pagination> {
  DramaNotifier([Pagination? state]) : super(state ?? Pagination.initial()) {
    getMovies();
  }
  ...
  Future<List<Movie>?> fetchMovies([int page = 1]) async {
    ...
    final String url =
        "$baseUrl$genre$apiKey&with_genres=18&page=$page&language=$langCode";
...
final dramaNotifier =
    StateNotifierProvider<DramaNotifier, Pagination>((ref) => DramaNotifier());
