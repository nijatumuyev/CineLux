...
class FantasyNotifier extends StateNotifier<Pagination> {
  FantasyNotifier([Pagination? state]) : super(state ?? Pagination.initial()) {
    getMovies();
  }
  ...
  Future<List<Movie>?> fetchMovies([int page = 1]) async {
    ...
    final String url =
        "$baseUrl$genre$apiKey&with_genres=14&page=$page&language=$langCode";
...
final fantasyNotifier = StateNotifierProvider<FantasyNotifier, Pagination>(
    (ref) => FantasyNotifier());
