...
class TvMovieNotifier extends StateNotifier<Pagination> {
  TvMovieNotifier([Pagination? state]) : super(state ?? Pagination.initial()) {
    getMovies();
  }
  ...
  Future<List<Movie>?> fetchMovies([int page = 1]) async {
    ...
    final String url =
        "$baseUrl$genre$apiKey&with_genres=10770&page=$page&language=$langCode";
...
final tvMovieNotifier = StateNotifierProvider<TvMovieNotifier, Pagination>(
    (ref) => TvMovieNotifier());
