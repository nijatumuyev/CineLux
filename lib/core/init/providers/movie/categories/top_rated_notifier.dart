...
class TopRatedNotifier extends StateNotifier<Pagination> {
  TopRatedNotifier([Pagination? state]) : super(state ?? Pagination.initial()) {
    getMovies();
  }

  Future<List<Movie>?> fetchMovies([int page = 1]) async {
    const String genre = "/movie/top_rated";

    final String url = "$baseUrl$genre$apiKey&page=$page&language=$langCode";
...
final topRatedNotifier = StateNotifierProvider<TopRatedNotifier, Pagination>(
