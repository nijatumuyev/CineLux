...
class WarNotifier extends StateNotifier<Pagination> {
  WarNotifier([Pagination? state]) : super(state ?? Pagination.initial()) {
    getMovies();
  }
  ...
  Future<List<Movie>?> fetchMovies([int page = 1]) async {
    ...
    final String url =
        "$baseUrl$genre$apiKey&with_genres=10752&page=$page&language=$langCode";
...
final warNotifier =
    StateNotifierProvider<WarNotifier, Pagination>((ref) => WarNotifier());
