...
class PopularNotifier extends StateNotifier<Pagination> {
  PopularNotifier([Pagination? state]) : super(state ?? Pagination.initial()) {
    getMovies();
  }

  Future<List<Movie>?> fetchMovies([int page = 1]) async {
    const String genre = "/movie/popular";
...
final popularNotifier = StateNotifierProvider<PopularNotifier, Pagination>(
    (ref) => PopularNotifier());
