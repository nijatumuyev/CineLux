...
class HorrorNotifier extends StateNotifier<Pagination> {
  HorrorNotifier([Pagination? state]) : super(state ?? Pagination.initial()) {
    getMovies();
  }
  ...
  Future<List<Movie>?> fetchMovies([int page = 1]) async {
    ...
    final String url =
        "$baseUrl$genre$apiKey&with_genres=27&page=$page&language=$langCode";
...
final horrorNotifier = StateNotifierProvider<HorrorNotifier, Pagination>(
    (ref) => HorrorNotifier());
