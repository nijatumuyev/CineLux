...
class FamilyNotifier extends StateNotifier<Pagination> {
  FamilyNotifier([Pagination? state]) : super(state ?? Pagination.initial()) {
    getMovies();
  }
  ...
  Future<List<Movie>?> fetchMovies([int page = 1]) async {
    ...
    final String url =
        "$baseUrl$genre$apiKey&with_genres=10751&page=$page&language=$langCode";
...
final familyNotifier = StateNotifierProvider<FamilyNotifier, Pagination>(
    (ref) => FamilyNotifier());
