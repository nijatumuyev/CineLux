...
class HistoryNotifier extends StateNotifier<Pagination> {
  HistoryNotifier([Pagination? state]) : super(state ?? Pagination.initial()) {
    getMovies();
  }
  ...
  Future<List<Movie>?> fetchMovies([int page = 1]) async {
    ...
    final String url =
        "$baseUrl$genre$apiKey&with_genres=36&page=$page&language=$langCode";
...
final historyNotifier = StateNotifierProvider<HistoryNotifier, Pagination>(
    (ref) => HistoryNotifier());
