...
class MysteryNotifier extends StateNotifier<Pagination> {
  MysteryNotifier([Pagination? state]) : super(state ?? Pagination.initial()) {
    getMovies();
  }
  ...
  Future<List<Movie>?> fetchMovies([int page = 1]) async {
    ...
    final String url =
        "$baseUrl$genre$apiKey&with_genres=9648&page=$page&language=$langCode";
...
final mysteryNotifier = StateNotifierProvider<MysteryNotifier, Pagination>(
    (ref) => MysteryNotifier());
