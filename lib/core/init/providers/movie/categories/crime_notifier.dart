...
class CrimeNotifier extends StateNotifier<Pagination> {
  CrimeNotifier([Pagination? state]) : super(state ?? Pagination.initial()) {
    getMovies();
  }
  ...
  Future<List<Movie>?> fetchMovies([int page = 1]) async {
    ...
    final String url =
        "$baseUrl$genre$apiKey&with_genres=80&page=$page&language=$langCode";
...
final crimeNotifier =
    StateNotifierProvider<CrimeNotifier, Pagination>((ref) => CrimeNotifier());
