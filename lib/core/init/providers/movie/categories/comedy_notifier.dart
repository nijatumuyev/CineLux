...
class ComedyNotifier extends StateNotifier<Pagination> {
  ComedyNotifier([Pagination? state]) : super(state ?? Pagination.initial()) {
    getMovies();
  }
  ...
  Future<List<Movie>?> fetchMovies([int page = 1]) async {
    ...
    final String url =
        "$baseUrl$genre$apiKey&with_genres=35&page=$page&language=$langCode";
...
final comedyNotifier = StateNotifierProvider<ComedyNotifier, Pagination>(
    (ref) => ComedyNotifier());
