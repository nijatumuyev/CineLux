...
class WesternNotifier extends StateNotifier<Pagination> {
  WesternNotifier([Pagination? state]) : super(state ?? Pagination.initial()) {
    getMovies();
  }
  ...
  Future<List<Movie>?> fetchMovies([int page = 1]) async {
    ...
    final String url =
        "$baseUrl$genre$apiKey&with_genres=37&page=$page&language=$langCode";
...
final westernNotifier = StateNotifierProvider<WesternNotifier, Pagination>(
    (ref) => WesternNotifier());
