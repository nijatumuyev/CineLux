...
class RomanceNotifier extends StateNotifier<Pagination> {
  RomanceNotifier([Pagination? state]) : super(state ?? Pagination.initial()) {
    getMovies();
  }
  ...
  Future<List<Movie>?> fetchMovies([int page = 1]) async {
    ...
    final String url =
        "$baseUrl$genre$apiKey&with_genres=10749&page=$page&language=$langCode";
...
final romanceNotifier = StateNotifierProvider<RomanceNotifier, Pagination>(
    (ref) => RomanceNotifier());
