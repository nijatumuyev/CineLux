...
class MusicNotifier extends StateNotifier<Pagination> {
  MusicNotifier([Pagination? state]) : super(state ?? Pagination.initial()) {
    getMovies();
  }
  ...
  Future<List<Movie>?> fetchMovies([int page = 1]) async {
    ...
    final String url =
        "$baseUrl$genre$apiKey&with_genres=10402&page=$page&language=$langCode";
...
final musicNotifier =
    StateNotifierProvider<MusicNotifier, Pagination>((ref) => MusicNotifier());
